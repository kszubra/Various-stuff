...
public class DifferenceColoringUtils {
    ...
    public void markRectangles(BufferedImage base, BufferedImage possiblyChanged, File destination, double tolerance, int numberOfClusters, int maxIterations) {
        if (isImageSizesNotEqual(base, possiblyChanged)) {
            System.out.println("Not same sizes");
        } else {

            List<ClusterRectangle> rectangles = ImageComparisonUtils.getClusters(base, possiblyChanged, tolerance, numberOfClusters, maxIterations);
