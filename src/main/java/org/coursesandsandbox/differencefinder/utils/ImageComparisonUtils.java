...
import org.coursesandsandbox.differencefinder.clustering.ClusterRecord;
import org.coursesandsandbox.differencefinder.clustering.KMeansClusterService;
...
public class ImageComparisonUtils {
    ...
    public static List<ClusterRectangle> getClusters(BufferedImage base, BufferedImage possiblyChanged, double toleranceLevel, int numberOfClusters, int maxIterations) {
        List<ClusterRecord> records = getClusterRecords(base, possiblyChanged, toleranceLevel);
        return KMeansClusterService.generateClusters(records, numberOfClusters, maxIterations).entrySet().stream()
